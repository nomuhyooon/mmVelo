...
For each condition the script:
  ...
  - Computes gene-wise / peak-wise Pearson correlation (train vs test)
  - Computes Velocity Consistency Score (VCS) and saves a boxplot
"""
...
from mmvelo_multi.dataset import SHARESeqHFDataModule_Pre, DynDataModule_Smooth
...
def make_streamline_plot(adata_r, adata_a, save_dir, clusters, umap_coord,
    ...
    adata_r.obs["clusters"] = clusters.values
...
# UMAP is loaded from the saved loom (X_umap stored by the original run).
_adata_ref = sc.read_loom(
    ANNDATA_DIR + "/adata_rna.loom",
    obs_names="obs_names", var_names="var_names",
)
umap_coord = _adata_ref.obsm["X_umap"]
del _adata_ref

clusters = pd.read_csv(
    ANNDATA_DIR + "/refined_clusters.tsv", sep="\t", header=None,
)[0].astype("category")
