...
for index, rows in filtered_network_df.iterrows():
    ...
    tf_expr, dadt = adata_r[:, tf].layers["s_raw"].reshape(-1).toarray(), adata_a[:, peak].layers["dadt"].reshape(-1).toarray()
...
filtered_network_df.to_csv(dir_path + '/network_filtered_fdr_1e-2_bh.tsv', sep='\t', header=True, index=False)
