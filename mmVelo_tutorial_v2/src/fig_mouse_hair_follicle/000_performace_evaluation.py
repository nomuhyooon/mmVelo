...
from mmvelo_multi.utils import fit_beta_gamma, plot_umap, plot_genewise_corr, plot_peakwise_corr, plot_size_factor, plot_vec_embed, fit_beta_gamma, get_filter_idx_raw, plot_su_expr_umap, plot_su_phase_raw, plot_genewise_vel_cossim, plot_su_phase_dsdt_var, plot_fluctuation_umap, fit_beta_gamma_scale, compute_cossim_genewise_contribution, plot_vec_embed_tanh, plot_genewise_vel_cossim_scale, plot_filtered_vec_embed, plot_genewise_corr_for_fig, plot_peakwise_corr_for_fig, plot_size_factor_for_fig
...
plot_genewise_corr_for_fig(dm_s, runPath, ms=True)
plot_genewise_corr_for_fig(dm_s, runPath, mu=True)
plot_peakwise_corr_for_fig(dm_s, runPath, ma=True)
...
#save_dir = runPath + "/downstream_analysis/result"
...
# atac Normal peak-wisevar
#np.savetxt(save_dir+"/Ma_peak_var.txt", torch.exp(model.logvar_a).numpy())
...
save_dir = runPath + "/downstream_analysis/result/streamplot"
#save_dir = runPath + "/downstream_analysis/result"
#save_dir = runPath + "/downstream_analysis"
if not os.path.exists(save_dir):
    os.mkdir(save_dir)
...
plot_umap(dm_s, save_dir, embedding=False, cluster_name="clusters")
#plot_vec_embed_tanh(dm_s, save_dir, latent=True, spliced=True, dsdt_obs=False, dadt=True, color="clusters")
#plot_filtered_vec_embed(dm_s, save_dir, filter_idx, spliced=True, dsdt_obs=False, ss_model=False, color="clusters")
...
def plot_vec_embed_tanh(dm, exp_name, latent=False, spliced=False, unspliced=False, dadt=False, color=None):
    if latent:
        ...
        scv.pl.velocity_embedding_stream(dm.adata_z, vkey="dynamics", color=color,save=exp_name+"/dzdt_streamline_" + color + "_tanh_with_legend.png", title="",
                                         dpi=300, legend_loc="right margin", )
        scv.pl.velocity_embedding_stream(dm.adata_z, vkey="dynamics", color=color,save=exp_name+"/dzdt_streamline_" + color + "_tanh.png", title="",
                                         dpi=300, legend_loc="none", )
        scv.pl.velocity_embedding_stream(dm.adata_z, vkey="dynamics", color=color,save=exp_name+"/dzdt_streamline_" + color + "_tanh_with_legend_on_data.png", title="",
    ...
    if spliced:
        ...
        scv.pl.velocity_embedding_stream(dm.adata_r, vkey="dsdt", color=color, save=exp_name+"/dsdt_streamline_" + color + "_tanh_with_legend.png",
                                         title="", dpi=300, legend_loc="right margin", )
        scv.pl.velocity_embedding_stream(dm.adata_r, vkey="dsdt", color=color, save=exp_name+"/dsdt_streamline_" + color + "_tanh.png",
                                         title="", dpi=300, legend_loc="none", )
        scv.pl.velocity_embedding_stream(dm.adata_r, vkey="dsdt", color=color,save=exp_name+"/dsdt_streamline_" + color + "_tanh_with_legend_on_data.png", title="",
                                         dpi=300, legend_loc="on data", )
    
    if unspliced:
        velocity_graph(dm.adata_r, vkey="dudt", xkey="u_raw", n_jobs=16)
        scv.tl.velocity_embedding(dm.adata_r, basis="umap", vkey="dudt")
        scv.pl.velocity_embedding_grid(dm.adata_r, vkey="dudt", color=color, save=exp_name+"/dudt_grid_" + color +"_tanh.png", title="dudt", dpi=300)
        scv.pl.velocity_embedding_stream(dm.adata_r, vkey="dudt", color=color, save=exp_name+"/dudt_streamline_" + color + "_tanh_with_legend.png",
                                         title="", dpi=300, legend_loc="right margin", )
        scv.pl.velocity_embedding_stream(dm.adata_r, vkey="dudt", color=color, save=exp_name+"/dudt_streamline_" + color + "_tanh.png",
                                         title="", dpi=300, legend_loc="none", )
        scv.pl.velocity_embedding_stream(dm.adata_r, vkey="dudt", color=color,save=exp_name+"/dudt_streamline_" + color + "_tanh_with_legend_on_data.png", title="",
                                         dpi=300, legend_loc="on data", )
    ...
    if dadt:
        ...
        scv.pl.velocity_embedding_stream(dm.adata_a, vkey="dadt", color=color, save=exp_name+"/dadt_streamline_" + color + "_tanh_with_legend.png",
                                         title="", dpi=300, legend_loc="right margin", )
        scv.pl.velocity_embedding_stream(dm.adata_a, vkey="dadt", color=color, save=exp_name+"/dadt_streamline_" + color + "_tanh.png",
                                         title="", dpi=300, legend_loc="none", )
        scv.pl.velocity_embedding_stream(dm.adata_a, vkey="dadt", color=color,save=exp_name+"/dadt_streamline_" + color + "_tanh_with_legend_on_data.png", title="",
                                         dpi=300, legend_loc="on data", )


plot_vec_embed_tanh(dm_s, save_dir, latent=True, color="clusters")
plot_vec_embed_tanh(dm_s, save_dir, spliced=True, color="clusters")
plot_vec_embed_tanh(dm_s, save_dir, unspliced=True, color="clusters")
plot_vec_embed_tanh(dm_s, save_dir, dadt=True, color="clusters")
