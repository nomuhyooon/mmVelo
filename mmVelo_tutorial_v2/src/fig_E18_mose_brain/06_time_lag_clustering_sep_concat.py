# gene 方向に p, u, s を concat して解析しようとした script
...
x_raw = np.concatenate([s_raw, u_raw, p_raw], axis=1) # 3735, 3327

gene_name = adata_r[:, prom_gene].var_names.to_numpy()
var_name = np.concatenate([adata_r[:, prom_gene].var_names.to_numpy() + "_s",
                           adata_r[:, prom_gene].var_names.to_numpy() + "_u",
                           adata_r[:, prom_gene].var_names.to_numpy() + "_p"])
...
for i in range(num_bins):
    ...
    bin_mean = np.mean(x_raw[bin_indices, :], axis=0)
    binned_data.append(bin_mean)

binned_x_raw = np.array(binned_data)
corr_mat = np.corrcoef(binned_x_raw, rowvar=False)
dist_mat = 1- corr_mat
...
sorted_binned_x_raw = scipy.stats.zscore(sorted_binned_x_raw)
...
fig.tight_layout()
plt.savefig(dir_path + "/heatmap_clustering_binned_x_{}.png".format(num_clusters), bbox_inches='tight')
#plt.savefig(dir_path + "/heatmap_clustering_binned_x.png", bbox_inches='tight')
plt.close()

# num_clusters = 4 is optimal
...
dxdt = np.concatenate([dsdt, dudt, dpdt], axis=1) # 3735, 3327
...
for i in range(num_bins):
    ...
    bin_end = percentiles[i+1]
    bin_indices = np.where((pseudotime_uni >= bin_start) & (pseudotime_uni < bin_end))[0]
    bin_mean = np.mean(dxdt[bin_indices, :], axis=0)
    binned_data.append(bin_mean)

binned_dxdt = np.array(binned_data)

cossim_mat = np.dot(binned_dxdt.T, binned_dxdt)
norms = np.linalg.norm(binned_dxdt, axis=0)
...
sorted_binned_dxdt = binned_dxdt[:, np.argsort(clusters)]
sorted_var_name = var_name[np.argsort(clusters)]
clusters = clusters[np.argsort(clusters)]
sorted_binned_dxdt = sorted_binned_dxdt / np.std(sorted_binned_dxdt, axis=0)
sorted_binned_dxdt = sorted_binned_dxdt.T
...
sns.set(font_scale=0.8)
fig, ax = plt.subplots(figsize=(10, 8))
sns.clustermap(sorted_binned_dxdt, cmap='coolwarm', xticklabels=False, yticklabels=False,
               row_cluster=False, col_cluster=False, row_colors=row_colors)

plt.xlabel('Pseudotime bins')
plt.title('Velocity Heatmap')
fig.tight_layout()
plt.savefig(dir_path + "/heatmap_clustering_binned_dx_{}.png".format(num_clusters), bbox_inches='tight')
#plt.savefig(dir_path + "/heatmap_clustering_binned_dx.png", bbox_inches='tight')
plt.close()


# clustering, by velocity and plot with count
s_raw = adata_r[:, prom_gene].layers["s_raw"].toarray() # 3735, 1109
u_raw = adata_r[:, prom_gene].layers["u_raw"].toarray()
p_raw = adata_r[:, prom_gene].layers["p_raw"]
x_raw = np.concatenate([s_raw, u_raw, p_raw], axis=1) # 3735, 3327
...
plt.title('Velocity Heatmap')
fig.tight_layout()
plt.savefig(dir_path + "/heatmap_clustering_binned_dx_{}_x.png".format(num_clusters), bbox_inches='tight')
