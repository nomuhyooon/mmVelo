...
sys.path.append("mmVelo_tutorial_/src/mmvelo_multi")
...
class DREG_PRE(pl.LightningModule):
    ...
    def set_grad_for_pretrain_second(self):
        ...
        '''
        self._pz_params[1].requires_grad = True
    ...
    def compute_normal_loss(self, x, ld, s_=False, u_=False, a_=False):
        ld, _ = ld[0], ld[1]
        if s_:
            px_z = dist.normal.Normal(ld * self.norm_mat_s, torch.exp(self.logvar_s))

        elif u_:
            px_z = dist.normal.Normal(ld * self.norm_mat_u, torch.exp(self.logvar_u))
            lpx_z = px_z.log_prob(x) * self.retain_gene_idx
            return lpx_z.sum(-1)
        
        elif a_:
            px_z = dist.normal.Normal(ld * self.norm_mat_a, torch.exp(self.logvar_a))
        else:
